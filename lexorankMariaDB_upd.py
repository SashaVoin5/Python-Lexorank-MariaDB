...
try:
    ...
    while True:
        ...
        print ("4.Add the new task on the start")
        print ("5.select * from Todo order by Priority desc")
        print ("6.select * from Todo order by Priority asc")
        print ("7.Exit for the programm")
        command = str(input())
        ...
        if (command == '2'):
            ...
            cur.execute(f"insert into Todo (Name,Comment,DeadLineDate,Priority,ExecutionStatusID) VALUES ('{name}','{Comment}','{DeadLineDate}','{outlex}','1')")
            Connection.commit()
        ...
        if ((command == '3')):
            ...
            cur.execute(f"insert into Todo (Name,Comment,DeadLineDate,Priority,ExecutionStatusID) VALUES ('{name}','{Comment}','{DeadLineDate}','{middleLexorank}','1')")
            Connection.commit()
        ...
        if ((command == '4')):
            ...
            cur.execute(f"SELECT Priority from Todo order by Priority asc")          
            results = cur.fetchall()           
            for row in results:
                Lexorank2 = str(row)
                break  
            Lexorank2 = Lexorank2[2:-3]            
            middleLexorank = lexorang('AAAAAA',Lexorank2)
            middleLexorank=middleLexorank.upper()           
        ...
        if ((command == '6')):
            cur.execute("select * from Todo order by Priority asc")
